...
/**
 * Returns the raw `Script` source text of an item as loaded from the item
 * database (used by the card collection album to re-apply a card's effect
 * through `bonus_script`). Returns nullptr when the item has no script.
 */
